 ...
 *
 * Each record: { title, href, section, text }. `text` is the FULL body
 * (or control frontmatter description + config) stripped of markdown/HTML
 * noise — not truncated, so a search for a deep term like `templateLock`
 * or `addButtonLabel` still matches even when it appears far down a page.
 * The whole index is a few tens of KB, fine to ship in the page payload.
 * The UI never displays `text` (only title + section), so there's no
 * snippet length to balance against.
 *
...
export function buildSearchIndex() {
	return loadAllDocs()
		...
		.map((d) => {
			...
			// Fold in config-option names so a search for an option key
			// (e.g. `collapsedTitle`) finds the control that has it.
			const fm = d.frontmatter || {};
			const source = d.kind === 'control'
				? [
						fm.description,
						...(Array.isArray(fm.supports) ? fm.supports : []),
						...(Array.isArray(fm.configOptions) ? fm.configOptions.map((o) => o && o.name).filter(Boolean) : []),
					].filter(Boolean).join(' ')
				: d.body || '';
			return {
				title: d.title,
...
/**
 * Build a plain-text meta description for a doc: its frontmatter `description`
 * if set, otherwise the first meaningful paragraph of the body with markdown
 * stripped, trimmed to ~160 chars on a word boundary. Used for <meta
 * description> + OpenGraph so search snippets and social cards aren't blank.
 *
 * @param {object} doc A doc from loadAllDocs().
 * @returns {string}
 */
export function docDescription(doc) {
	if (!doc) return '';
	...
	// First paragraph = text up to the first blank line, skipping frontmatter,
	// headings, fences, directives, blockquotes, and list markers.
	let firstPara = '';
	...
	// Strip inline markdown noise: links -> text, code ticks, emphasis, tags.
